...
const lessonData = [
    {
        ...
        slides: [
            {
                title: '코더(Coder)에서 디렉터(Director)로 🎬',
                diagram: '👨‍💻 과거: [인간] ➡️ 영어 기반의 복잡한 문법 학습 ➡️ [컴퓨터]\n👨‍🎨 현재: [인간: 아이디어/자연어] ➡️ 🤖 AI가 코딩 통역사 역할 ➡️ [컴퓨터]',
                desc: '- **자연어가 프로그래밍 언어가 되다:** 과거에는 컴퓨터와 대화하기 위해 C언어, 파이썬 등 컴퓨터의 언어를 우리가 배워야 했습니다. 하지만 이제는 우리가 평소에 쓰는 \'한국어(자연어)\'로 지시하면, AI가 이를 컴퓨터 언어로 실시간 번역해 줍니다.\n- **디렉터의 마인드셋:** 영화감독이 카메라 조작법이나 조명 설치법을 몰라도 훌륭한 영화를 만들 수 있듯, 우리도 코드를 한 줄 한 줄 타이핑하는 \'작업자\'가 아니라, 어떤 기능을 만들지 기획하고 AI에게 방향을 지시하는 \'총괄 디렉터\'가 되어야 합니다.'
            },
            {
                title: 'C언어를 몰라도 괜찮은 이유 🎼',
                diagram: '🧠 명확한 기획 (무엇을 만들 것인가) ➕ 🗣️ 정교한 프롬프트 (어떻게 지시할 것인가) 🟰 💻 완벽히 구동되는 코드',
                desc: '- **에러의 공포에서 해방:** 예전에는 세미콜론(;) 하나, 오타 하나만 있어도 프로그램이 멈췄고, 초보자들은 여기서 포기했습니다.\n- **AI의 자가 수정:** 바이브 코딩 시대의 AI는 단순한 자동완성이 아닙니다. 에러가 나면 에러 메시지를 그대로 복사해서 "이거 에러 나는데 고쳐줘"라고 하면 스스로 원인을 파악하고 코드를 수정합니다. 우리는 문법 암기 대신 \'논리적인 생각\'에만 집중하면 됩니다.'
            },
            {
                title: '누구나 창작자가 되는 교실 🏫✨',
                diagram: '💡 교사: "반복 업무가 귀찮네" ➡️ AI 맞춤형 툴 즉시 제작 ➡️ 행정/수업 준비 시간 단축\n🌱 학생: "이런 게임이 있으면 좋겠다" ➡️ 상상을 현실로 구현 ➡️ 압도적인 성취감 획득',
                desc: '- **소프트웨어 교육의 혁명:** 코딩 교육의 진입 장벽이 완전히 무너졌습니다. 코딩 문법을 가르치느라 한 학기를 보내는 대신, 학생들은 첫 시간부터 자신의 아이디어를 소프트웨어로 만들어내는 경험을 하게 됩니다.\n- **교사를 위한 슈퍼파워:** 학급 규칙 뽑기, 학생 상담 기록기, 맞춤형 퀴즈 생성기 등 \'내 입맛에 딱 맞는\' 도구를 시중에서 찾을 필요 없이 그 자리에서 뚝딱 만들어 사용할 수 있습니다.'
            }
        ],
        qaList: [
    ...
    },
    {
        ...
        slides: [
            {
                title: '텍스트를 넘어 코드의 세계로 🚀',
                diagram: '📝 1단계: 텍스트 (제미나이 등 - 글짓기, 요약, 번역)\n🎨 2단계: 미디어 (나노바나나, VEO 등 - 이미지 및 영상 생성)\n💻 3단계: 로직과 소프트웨어 (바이브 코딩 - 동작하는 앱/웹 생성)',
                desc: '- AI 기술은 단순히 그럴듯한 텍스트나 이미지 \'보여주는\' 단계를 넘어섰습니다. 이제는 버튼을 누르면 데이터가 계산되고 화면이 넘어가는 등, 사용자와 상호작용하는 \'소프트웨어의 뼈대와 논리(Logic)\'를 통째로 생성해 냅니다.'
            },
            {
                title: '상상이 현실이 되는 마법 🪄',
                diagram: '☁️ 상상: "우리 반 아이들 칭찬 도장 찍어주는 앱이 있으면 좋겠어!"\n⬇️ (AI와 채팅하며 요구사항 수정 및 발전)\n📱 현실: 실제 내 스마트폰 브라우저에 깔려 있는 나만의 \'칭찬 앱\'',
                desc: '- 기존에는 앱 하나를 만들려면 외주를 맡겨 수백만 원을 쓰거나, 몇 달간 개발 학원을 다녀야 했습니다.\n- 이제는 커피 한 잔 마시는 시간 동안 AI와 대화하며, 내가 교실에서 당장 필요한 맞춤형 앱의 프로토타입(시제품)을 만들어내고 실사용까지 할 수 있습니다.'
            },
            {
                title: '바이브 코딩으로 가능한 창작물 🏆',
                diagram: '1️⃣ 🌐 웹페이지: 텍스트, 이미지, 오디오, 동영상 총 집합체\n2️⃣ 📚 도서 출판: 텍스트, 이미지 사용',
                desc: '- **융합형 창작의 실현:** 바이브 코딩은 단순히 \'코드\'를 짜는 도구가 아닙니다. 텍스트와 멀티미디어가 결합된 고차원 웹 서비스는 물론, AI와 협업하여 집필한 지식 콘텐츠를 도서 형태로 출판하는 등 창작의 경계를 허뭅니다.\n- **지속 가능한 포트폴리오:** 연수 기간 내내 여러분이 직접 디렉팅한 결과물들은 단순한 연습이 아닌, 고유한 인터넷 주소(URL)를 가진 실제 서비스와 디지털 자산으로 남아 여러분의 전문성을 입증하게 됩니다.'
            }
        ],
        qaList: [
    ...
    },
    {
        ...
        slides: [
            {
                title: '식당으로 이해하는 웹의 작동 방식 🍽️',
                diagram: '👤 손님 (사용자/브라우저): "메뉴판 보고 떡볶이 주문할게요!" (클릭/요청)\n💁 홀서빙 (네트워크/API): 주문 내역을 주방으로 안전하게 전달 (통신)\n👨‍🍳 주방장 (서버/DB): 냉장고에서 재료 꺼내 요리 완성 후 서빙 (데이터 처리)',
                desc: '- 우리가 인터넷을 쓸 때 일어나는 일은 식당의 주문 과정과 똑같습니다. 브라우저(크롬)에서 버튼을 누르는 것은 손님이 주문하는 것이고, 이 주문이 보이지 않는 인터넷 망(서빙)을 타고 서버(주방)로 가서 데이터를 가져오는 끊임없는 티키타카의 연속입니다.'
            },
            {
                title: '눈에 보이는 화려한 무대 🎭 (Front-end)',
                diagram: '🏗️ HTML (뼈대): 텍스트, 이미지, 버튼 배치 (건물의 콘크리트)\n🎨 CSS (꾸미기): 폰트, 색상, 애니메이션 (건물의 페인트)\n⚙️ JavaScript (근육): 클릭 시 팝업 등 동작 (건물의 전기/수도)',
                desc: '- **프론트엔드(Front-end)**는 사용자의 눈에 보이고 직접 만지는 모든 화면 영역입니다. AI에게 화면 디자인을 지시할 때 이 세 가지 요소의 역할을 알면 훨씬 디테일하게 명령할 수 있습니다.',
                links: [{ name: 'MDN Web Docs (웹 기술 참고서)', url: 'https://developer.mozilla.org/' }]
            },
            {
                title: '무대 뒤에서 묵묵히 일하는 스태프 🥷 (Back-end)',
                diagram: '💾 데이터베이스 (DB): 회원 정보와 게시글 보관 창고\n🧠 AI/서버 연동: 외부 AI에 데이터를 물어보고 안전하게 응답만 전달하기',
                desc: '- **백엔드(Back-end)**는 눈에 보이지 않는 \'뒷단\'입니다. 로그인 정보를 기억하거나 데이터를 안전하게 저장하려면 백엔드가 필수적입니다.'
            }
        ],
        qaList: [
    ...
    },
    {
        ...
        slides: [
            {
                title: '개발자를 위한 강력한 AI 샌드박스 🛠️',
                diagram: '[복잡한 개발 환경 설치] ❌ ➡️ [크롬 브라우저 접속만으로 끝] ⭕',
                desc: '- 코딩을 처음 시작할 때 가장 큰 벽은 \'개발 프로그램 설치\'입니다. 구글 AI 스튜디오는 웹 사이트 접속만으로 구글의 최신 모델(Gemini)을 개발자 수준으로 세밀하게 조작할 수 있는 완벽한 샌드박스입니다.',
                links: [{ name: 'Google AI Studio 바로가기', url: 'https://aistudio.google.com/' }]
            },
            {
                title: 'AI를 똑똑하게 부려먹는 마법의 주문 📜',
                diagram: '❌ 하수: "재밌는 퀴즈 게임 하나 짜줘."\n⭕ 고수: [페르소나] + [구체적 규칙] + [출력 형태]',
                desc: '- AI는 질문의 수준에 맞춰 대답합니다. 역할을 부여하고(너는 개발자야), 예외 상황이나 규칙을 구체적으로 통제할수록 완벽한 결과물을 얻을 수 있습니다.'
            },
            {
                title: '클릭 세 번으로 내 웹사이트 탄생 🐣',
                diagram: '1️⃣ 시스템 프롬프트(지시문) 입력 ➡️ 2️⃣ \'Get Code(코드 보기)\' 클릭 ➡️ 3️⃣ 생성된 코드를 복사해서 실행!',
                desc: '- 구글 AI 스튜디오 안에서는 내가 짠 프롬프트가 실제 어떤 코드로 변환되는지 실시간 확인 가능합니다.'
            }
        ],
        qaList: [
            '구글 AI 스튜디오 실전 사용법은?(기능, 제작, 배포, 공유 등)',
    ...
    },
    {
        ...
        slides: [
            {
                title: '본격적인 개발을 위한 나만의 작업실 🏗️',
                diagram: '📝 메모장 (오타 안 잡아줌 🚫) ➡️ 💻 코드 에디터 (색상 구분, 자동 완성, 에러 체크 ✅)',
                desc: '- 전문 목수에게 좋은 톱이 필요하듯, 우리에게는 코드를 정렬해주고 실수를 방지해 주는 \'전용 편집기(Code Editor)\'가 필수입니다.'
            },
            {
                title: '대표적인 에디터 3대장, 내 취향 고르기 ⚔️',
                diagram: '📘 VS Code: 전 세계 1위. 정석.\n🚀 Windsurf (윈드서프): 요즘 가장 뜨거운 AI 네이티브 에디터.\n🖱️ Cursor (커서): AI 코딩 자동화의 양대산맥 끝판왕.',
                desc: '- 기존에 익숙한 방식이 좋다면 VS Code를, AI가 내 코드를 전반적으로 파악하고 타이핑해 주는 미래지향적 경험을 원한다면 Cursor나 Windsurf를 추천합니다.',
                links: [
                    { name: 'VS Code 다운로드', url: 'https://code.visualstudio.com/' },
                    { name: 'Windsurf 다운로드', url: 'https://codeium.com/windsurf' },
                    { name: 'Cursor 다운로드', url: 'https://www.cursor.com/' }
                ]
            },
            {
                title: '작업실 인테리어 시작하기 (초기 세팅) 🪚',
                diagram: '설치 ➡️ 바탕화면에 \'바이브코딩_작업실\' 빈 폴더 생성 ➡️ 에디터에서 해당 폴더 열기',
                desc: '- 에디터에게 \'내가 작업할 공간(폴더)\'을 지정해 주어야 합니다. index.html 파일을 생성하는 것부터가 시작입니다.'
            }
        ],
        qaList: [
            '바이브 코딩 수준별로 적합한 에디터는?',
            'Cursor나 Windsurf 같은 AI 에디터는 유료 결제 없이도 연수를 따라가기에 충분한가요?',
    ...
    },
    {
        ...
        slides: [
            {
                title: '내 작업실에 상주하는 24시간 코딩 비서 🤵‍♂️',
                diagram: '웹 브라우저 복붙 (번거로움 폭발) ➡️ 에디터 속 AI 에이전트 도입 (파일 문맥 자동 파악)',
                desc: '- 웹 브라우저와 에디터 창을 오갈 필요가 없습니다. 내 코딩 작업실 안에 AI를 직접 고용하여 파일 전체 맥락을 파악하게 합니다.'
            },
            {
                title: '대화하며 코딩하는 법 (인라인 챗 & 사이드바) 💬',
                diagram: '코드 드래그 ➡️ 단축키(Ctrl+L 등) 입력 ➡️ "이거 배경색 바꿔줘" ➡️ \'적용(Apply)\' 클릭!',
                desc: '- 코드를 직접 치지 마세요. 지시만 내리면 AI가 기존 코드와 새 코드를 비교(Diff)해서 보여줍니다. 마음에 들면 수락(Accept)만 누르세요.'
            },
            {
                title: 'Composer / 에이전트 모드 활용 🤖',
                diagram: '여러 개의 파일을 동시에 생성하고 연결하는 강력한 AI 자율 모드 경험하기',
                desc: '- 파일 하나를 넘어 폴더 구조 자체를 AI가 기획하고 여러 파일을 동시에 수정하도록 지시하는 심화 기능을 맛봅니다.'
            }
        ],
        qaList: [
    ...
    },
    {
        ...
        slides: [
            {
                title: '개발자들의 구글 드라이브이자 SNS 🐙',
                diagram: '💻 내 컴퓨터 (고장 시 증발) ➡️ ☁️ 깃허브 (인터넷 금고에 안전 백업)',
                desc: '- 내가 작성한 코드 파일들을 클라우드에 업로드하여 언제 어디서나 불러오게 해주는 필수 플랫폼입니다.',
                links: [{ name: 'GitHub 공식 사이트', url: 'https://github.com/' }]
            },
            {
                title: '코드의 타임머신 탑승하기 (버전 관리) ⏳',
                diagram: '버전1(정상) ➡️ 버전2(수정) ➡️ "망했다, 버전1로 돌려줘!" ➡️ 원상복구',
                desc: '- 깃허브는 특정 시점의 상태를 \'사진(Snapshot)\'처럼 찍어두기 때문에, 치명적인 실수를 해도 언제든 과거로 돌아갈 수 있는 든든한 보험입니다.'
            },
            {
                title: '향후 무료 웹 배포를 위한 첫 관문 🚪',
                diagram: '에디터 연동 ➡️ \'Commit(저장)\' & \'Push(업로드)\' ➡️ 깃허브 업로드 확인',
                desc: '- 깃허브에 코드를 올려두어야만 추후 Vercel 같은 무료 배포 플랫폼과 연결해 전 세계에 사이트를 띄울 수 있습니다.'
            }
        ],
        qaList: [
            '깃허브의 기본 사용법은? (주요용어와 기능)',
            ...
            '깃허브에서 배포하는 절차는?',
            '깃허브는 왜 무료인가요?',
            '깃허브를 이용하여 직장, 집, 카페 등 여러 장소에서 작업을 이어나가는 방법은?'
        ]
    },
    {
        id: 8,
        ...
        slides: [
            {
                title: '서버 비용 0원! 깃털처럼 가볍고 빠른 웹 🪶',
                diagram: '동적 웹 (서버 연산/비용 발생) 🆚 정적 웹 (HTML/JS만 존재, 유지비 무료)',
            ...
            },
            {
                ...
                diagram: '코드 수정 ➡️ 저장(Ctrl+S) ➡️ 브라우저 새로고침(F5) ➡️ 즉시 확인',
                desc: '- 프론트엔드 개발의 즐거움은 내 수정 사항이 눈앞에 시각적으로 바로 나타난다는 점입니다.'
            }
        ],
        qaList: [
    ...
    },
    {
        ...
        slides: [
            {
                title: '질문을 대신 조립해주는 영리한 자판기 🧃',
                diagram: '[학년 선택] ➕ [과목 선택] 🟰 자동 완성된 정교한 프롬프트 문장',
                desc: '- 화면에 보기 편한 드롭다운을 만들면, 자바스크립트가 완벽한 프롬프트 문장으로 조립해 주는 똑똑한 인터페이스를 만듭니다.'
            },
            {
                title: '내 돈 한 푼 안 쓰는 \'꼼수\' 연동 로직 🤫',
                diagram: '문장 완성 ➡️ 클립보드 자동 복사 📋 ➡️ 챗GPT 공식 홈페이지 강제 새창 열기 🚀',
    ...
    },
    {
        ...
        slides: [
            {
                title: '내 웹과 AI를 연결하는 보이지 않는 파이프라인 🚰',
                diagram: '🙋‍♂️ 내 웹사이트 ↔️ 💁‍♀️ API (서빙 점원) ↔️ 🤖 AI 서버 (주방)',
                desc: '- 외부의 똑똑한 AI 컴퓨터와 내 웹이 데이터를 주고받을 수 있도록 규격화된 통신 창구가 바로 API입니다.'
            },
            {
                title: 'AI 세상의 화폐 단위, 토큰(Token) 🪙',
                diagram: '1 Token 🟰 영어 약 0.75단어 / 한글 약 0.5글자 (한글이 과금이 더 비쌈)',
                desc: '- AI 기업들은 데이터를 처리한 양만큼 요금을 받으며, 그 기준 단위가 \'토큰\'입니다.'
            },
            {
                title: '가성비 vs 성능, 목적에 맞는 무기 선택 ⚖️',
                diagram: '🥇 성능 끝판왕: GPT-4o, Claude 3.5 Sonnet\n🏃 가성비 끝판왕: GPT-4o-mini, Gemini 1.5 Flash',
                desc: '- 내가 만들 웹 서비스의 난이도에 맞춰 가성비 모델을 영리하게 선택해야 합니다.'
            }
        ],
        qaList: [
            '구글 신규 계정에서 제공하는 API 무료 혜택이 있나요?',
    ...
    },
    {
        ...
        slides: [
            {
                title: '지갑을 열지 않고 테스트하기 (구글 신규 혜택) 🎁',
                diagram: '🎉 Gemini 1.5 Flash 모델: 1분 15번, 하루 1,500번 요청 전면 무료!',
                desc: '- 구글의 제미나이 생태계는 개인 학습자나 테스트 목적의 개발자들에게 매우 넉넉한 영구 무료 한도를 제공합니다.'
            },
            {
                title: '내 신용카드를 버리지 마세요 (매우 중요!) 💳🚨',
                diagram: '❌ 코드에 대놓고 적기 ➡️ 해킹 표적\n⭕ .env 비밀 파일에 숨기고 .gitignore 처리하기',
                desc: '- API Key를 코드에 적나라하게 쓴 채 깃허브에 올리면 해커들이 1초 만에 긁어가서 요금 폭탄을 맞습니다. 보안은 1원칙입니다.'
            },
            {
                title: '안전벨트 꽉 매기 (과다청구 방지) 🛑',
                diagram: '결제 대시보드 ➡️ Hard Limit(결제 차단선) 월 5달러 강제 설정',
                desc: '- 시스템 자체적으로 결제 상한선을 설정해 두면 안심하고 코딩할 수 있습니다.'
            }
        ],
        qaList: [
            '내가 배포한 웹페이지 코드에 내 API가 포함되면, 어떤 일이 발생하나요?',
            '(구글 기준) 나의 API 사용량 조회하는 방법은?',
    ...
    },
    {
        ...
        slides: [
            {
                title: '유지비 Zero 전략의 꽃, "네 API를 쓰렴" 🌸',
                diagram: '프론트엔드 UI 제공(나) ➕ AI 사용료 부담(사용자) 🟰 내 서버비 0원!',
                desc: '- 개발자는 껍데기만 제공하고, AI 통신 요금은 각 사용자가 본인의 지갑(API 키)을 열어 지불하게 만드는 경제적인 방식입니다.'
            },
            {
                title: '매번 입력하는 귀찮음 없애기 (Local Storage) 💾',
                diagram: '브라우저 내부 창고(Local Storage)에 키를 암호화하여 임시 저장',
    ...
    },
    {
        ...
        slides: [
            {
                title: '진짜 상용 서비스처럼 매끄럽게 만들기 🏢',
                diagram: '[프론트엔드 화면] ↔️ [내 백엔드: API 키 금고] ↔️ [AI 모델]',
                desc: '- 사용자가 키를 입력할 필요 없이 바로 결과가 나오는 완벽한 서비스 형태입니다. 단, 비용은 내 API 키에서 나갑니다.'
            },
            {
                title: '프론트와 AI 사이에 검문소(서버) 세우기 🚧',
                diagram: '프론트 코드에 키 노출 방지 ➡️ 안전한 백엔드 서버 경유',
                desc: '- 내 소유의 안전한 백엔드를 구축하여 파이프라인을 꺾어주어야만 키 해킹을 방지할 수 있습니다.'
            },
            {
                title: '요금 폭탄 막는 강력한 방패 (Rate Limit & 캐싱) 🛡️',
                diagram: 'IP당 횟수 제한(Rate Limit) & 중복 질문 답변 복붙(Caching)',
                desc: '- 과금을 방어하기 위해 트래픽을 통제하는 서버측 방어 로직을 바이브 코딩으로 쉽게 구현합니다.'
            }
        ],
        qaList: [
            '기업도 아닌 개인이 본인 API를 포함한 웹페이지를 배포할 만한 이유가 있나요?',
    ...
    },
    {
        ...
        slides: [
            {
                title: '에러는 실패가 아닌 친절한 이정표 🚨',
                diagram: '화면이 백지 상태 ➡️ 당황하여 코드 전체 삭제 (하수) ➡️ 브라우저 "개발자 도구(F12)"를 열어 빨간 글씨 확인 (고수)',
                desc: '- 에러가 났을 때는 AI에게 무작정 "안 돼"라고 하기보다 브라우저 콘솔창의 빨간색 에러 로그를 복사해 주어야 AI가 단번에 해결합니다.'
            },
            {
                title: '가장 효과적인 디버깅 프롬프트 팁 🩹',
                diagram: '"[기대했던 동작]을 하려고 했는데, [현재 발생하는 현상]이 발생해. 에러 메시지는 [복사한 로그]야. 원인과 해결 코드를 알려줘."',
                desc: '- AI에게 증상을 의사에게 설명하듯 명확히 전달하는 프롬프팅이 필요합니다. 에디터 안에서 해당 파일을 지정(예: @index.html)하고 질문하는 것이 핵심입니다.'
            },
            {
                title: '점진적 수정과 롤백(Rollback)의 미학 🔄',
                diagram: '한 번에 너무 많은 기능 추가 요구 ➡️ 전체 코드 붕괴 ➡️ Git을 통해 직전 정상 상태로 복구(롤백) 후 하나씩 재요구',
                desc: '- AI에게 한 번에 너무 거대한 수정을 요구하면 구조가 꼬입니다. 작은 단위로 기능을 쪼개어 테스트하고, 망가졌을 때는 망설임 없이 Git으로 뒤로 돌아가는 용기가 필요합니다.'
            }
        ],
        qaList: [
    ...
    },
    {
        ...
        slides: [
            {
                title: '새로고침 해도 데이터가 날아가지 않으려면? 💾',
                diagram: '로컬 스토리지 (내 폰에만 저장, 다른 기기 연동 불가) 🆚 클라우드 데이터베이스 (Supabase / Firebase - 모든 기기에서 데이터 공유)',
                desc: '- 진정한 웹/앱 서비스가 되려면 사용자가 남긴 방명록, 퀴즈 점수, 게시글 등이 영구적으로 저장되는 DB가 반드시 필요합니다.',
                links: [
                    { name: 'Supabase 공식 사이트', url: 'https://supabase.com/' },
                    { name: 'Firebase 공식 사이트', url: 'https://firebase.google.com/' }
                ]
            },
            {
                title: '서버 없는 서버 개발, BaaS의 마법 ☁️',
                diagram: '회원가입/로그인 로직 + 데이터베이스 구축 (몇 달 걸림) ➡️ BaaS 서비스 활용 (마우스 클릭 몇 번으로 DB와 API 자동 생성)',
                desc: '- Supabase(수파베이스)나 Firebase(파이어베이스) 같은 BaaS를 활용하면 초보자도 10분 만에 강력한 백엔드와 데이터베이스를 확보할 수 있습니다.'
            },
            {
                title: 'AI에게 DB 연동 지시하기 🤝',
                diagram: '1️⃣ Supabase에서 테이블(표) 만들기 ➡️ 2️⃣ API 주소/키 복사 ➡️ 3️⃣ AI 에이전트에게 "내 웹에서 버튼 누르면 이 Supabase 테이블에 데이터 기록하게 코드 짜줘" 지시',
                desc: '- 테이블(엑셀 표와 유사)을 구조화하는 법을 배우고, 프론트엔드 코드와 DB를 연결하는 코드를 AI에게 안전하게 위임하는 방법을 실습합니다.'
            }
        ],
        qaList: [
            '구글 스프레드시트를 데이터베이스(DB) 대신 사용하도록 AI에게 지시할 수도 있나요?',
            '직접 서버를 구축하지 않고 사용하는 구독형 데이터베이스 서비스가 있나요?',
    ...
    },
    {
        ...
        slides: [
            {
                title: '배포(Deployment)란? 내 컴퓨터 밖으로 나가기 🌍',
                diagram: '🏠 배포 전: 내 노트북 꺼지면 끝 ➡️ 🌐 배포 후: 24시간 켜져 있는 클라우드, URL 접속 가능',
                desc: '- 내 컴퓨터 안에만 있는 프로젝트를 거대한 데이터센터로 옮기고 고유 인터넷 주소를 부여받는 과정입니다.'
            },
            {
                title: '클릭 몇 번이면 끝나는 마법, Vercel(버셀) ⚡',
                diagram: '가입 ➡️ 깃허브 연결 ➡️ 배포할 프로젝트 \'Import\' ➡️ (1분 대기) 라이브 웹사이트 탄생!',
                desc: '- Vercel은 복잡한 서버 세팅을 생략하고 소스코드만 연동하면 알아서 서버 세팅해 줍니다.',
                links: [{ name: 'Vercel 공식 사이트', url: 'https://vercel.com/' }]
            },
            {
                title: '짜잔! 자동 업데이트 (CI/CD) 🎀',
                diagram: '코드를 수정하고 깃허브에 다시 올림 ➡️ Vercel이 알아채고 라이브 사이트에 1분 만에 자동 반영!',
                desc: '- 최초 1회만 연동해 두면, 이후 깃허브 업데이트 시 자동으로 새 버전으로 사이트가 교체됩니다.'
            }
        ],
        qaList: [
    ...
    },
    {
        ...
        slides: [
            {
                title: '복잡한 기본 주소 대신, 나만의 간판 달기 📛',
                diagram: '❌ Vercel 기본 주소 (vibe-project-random.vercel.app) ➡️ ⭕ 개인 도메인 (www.crownyuja.co.kr)',
                desc: '- 무료 배포 주소는 임시 번호판입니다. 깔끔한 도메인을 연결하는 순간 진짜 상용 서비스의 브랜드 가치를 갖습니다.'
            },
            {
                title: '내 땅(인터넷 주소) 저렴하게 등기 치기 📜',
                diagram: '국내 등록 업체(호스팅KR, 가비아 등) 검색 ➡️ 원하는 이름 1년 대여 결제',
                desc: '- .com, .co.kr 등 원하는 확장자를 저렴하게 확보해 보세요.',
                links: [
                    { name: '가비아 도메인 검색', url: 'https://domain.gabia.com/' },
                    { name: '호스팅KR 도메인 검색', url: 'https://www.hosting.kr/' }
                ]
            },
            {
                title: '산 도메인을 내 웹사이트에 씌우기 (네임서버 연결) 🔗',
                diagram: '[가비아 네임서버 설정] "이 주소 치면 Vercel 서버로 안내해 줘!" (매핑)',
                desc: '- 도메인 구매처와 서버 호스팅처 방향을 맞춰주는 \'네임서버(DNS)\' 설정으로 론칭을 최종 완료합니다.'
            }
        ],
        qaList: [
            '도메인 구매 시 주의사항은?',
            '구매한 도메인을 깃허브에서 연결하는 방법은?',
            ...
            <header class="px-4 md:px-6 py-4 md:py-5 cursor-pointer flex items-center justify-between" onclick="toggleAccordion(${lesson.id})">
                ...
                <div class="flex items-center gap-2 md:gap-3 shrink-0 ml-2">
                    <div class="hidden md:flex items-center gap-1.5 p-1.5 bg-slate-50 rounded-2xl border border-slate-100" onclick="event.stopPropagation()">
                        ...
                        <button onclick="handleMentoring(${lesson.id})" class="flex items-center gap-2 px-3 py-1.5 hover:bg-indigo-600 bg-indigo-50 rounded-xl transition-all group/btn">
                            <i data-lucide="bot" class="w-4 h-4 md:w-5 md:h-5 text-indigo-600 group-hover/btn:text-white transition-colors"></i>
                            <span class="text-xs font-black text-indigo-600 group-hover/btn:text-white transition-colors">AI 멘토</span>
                        </button>
                    </div>
                ...
                <div class="pt-4 border-t border-slate-50">
                    <div class="flex md:hidden items-center justify-between gap-1.5 p-2 bg-slate-50 rounded-xl border border-slate-100 mb-4">
                        ...
                        <div class="flex gap-1">
                            <button onclick="handleMentoring(${lesson.id}, true)" class="flex items-center gap-2 px-3 py-2 bg-indigo-100 text-indigo-600 rounded-lg">
                                <i data-lucide="bot" class="w-4 h-4"></i>
                                <span class="text-xs font-black">AI 멘토</span>
                            </button>
                        </div>
                    </div>
...
/**
 * 모든 슬라이드에서 링크를 수집하여 분야별로 정리
 */
function initLinksPopup() {
    const linksPopupContent = document.getElementById('links-popup-content');
    if (!linksPopupContent) return;
    ...
    lessonData.forEach(lesson => {
        lesson.slides.forEach(slide => {
            if (slide.links && slide.links.length > 0) {
                const topic = lesson.topic || '기타 자료';
                if (!linksByTopic[topic]) {
                    linksByTopic[topic] = [];
                }
                slide.links.forEach(link => {
                    const isDuplicate = linksByTopic[topic].some(l => l.url === link.url);
                    if (!isDuplicate) {
                        linksByTopic[topic].push(link);
                    }
                });
            }
        });
    ...
    if (topics.length > 0) {
        topics.forEach(topic => {
            const links = linksByTopic[topic];
            html += `
                <div class="mb-4 bg-white/50 rounded-2xl border border-slate-100 overflow-hidden">
                    <div class="px-4 py-2.5 bg-indigo-50/50 border-b border-indigo-100/50 flex items-center justify-between">
                        ...
                        <span class="text-[9px] font-bold text-indigo-400 bg-white px-2 py-0.5 rounded-full border border-indigo-50">${links.length}</span>
                    </div>
                    <div class="p-1 grid gap-1">
                        ${links.map(link => `
                            <a href="${link.url}" target="_blank" 
                                class="flex items-center justify-between px-3 py-3 hover:bg-white hover:shadow-sm rounded-xl transition-all group/link border border-transparent hover:border-slate-100">
                                <div class="flex flex-col gap-0.5">
                                    <span class="text-[13px] font-bold text-slate-700 group-hover/link:text-indigo-600 transition-colors line-clamp-1">${link.name}</span>
                                    <span class="text-[10px] text-slate-400 font-medium truncate max-w-[180px]">${link.url.replace('https://', '')}</span>
                                </div>
                                <div class="w-8 h-8 flex items-center justify-center rounded-lg bg-slate-50 group-hover/link:bg-indigo-50 transition-colors shrink-0">
                                    <i data-lucide="external-link" class="w-3.5 h-3.5 text-slate-400 group-hover/link:text-indigo-600"></i>
                                </div>
                            </a>
                        `).join('')}
                    </div>
        ...
        html = '<div class="py-20 text-center text-slate-300 font-bold">등록된 관련 링크가 없습니다.</div>';
    }

    linksPopupContent.innerHTML = html;
...
if (document.readyState === 'complete') {
    ...
    window.addEventListener('load', initLinksPopup);
}